...
**Answer:**
First, the result from the single inner_prod(v,w) run is 1022, not 1023.
Second, the result from the for loop run is not consistent and gives the error message.
The cause of these problems comes from indexing in the for loop. C++ indexing starts from 0, not 1 
as in R. Therefore, we need to change to i = 0 in C++ code. Furthermore, the loop should end at 1022, not 1023.
So, it should be < v.size(), not <= v.size(). After these fixes, the indexing starts from 0 to 1022 = 1023 times in C++ which gives a correct answer consistently. 
...
**Answer:**
The output matrix had a 4x2 dimension in which the first column was all 1024 and the second column was all the same big number. 
When I re-ran the whole code chunk, the numbers in the second column changed (they were all the same), but those in the first column stayed 1024. 
I checked matrix A and B in the c++ and the matrices looked correct until the A * B operation. 
It seems that Eigen "*" operation attempted to do matrix multiplication without checking the inner dimensions.
Each row of A with a length of 1024 was dotted with a column of B with length 512 + 512 (all 1s), giving the first column of 1024s. 
However, the second column of B was undefined, so the second column in the resulting matrix had unpredictable numbers.
...
***Answer:***
The RStudio crashed ! When run on R terminal, it said "***caught segfault***" address 0x134900000, cause 'invalid permissions.' 
This happened because vector x is larger than vector y, causing an out-of-bounds memory access and leading to a segmentation fault. 
The second code chunk runs fine because now vector x is smaller than vector y. To make `axpy_c.cpp` safer, add lines that 
check if the sizes of vector x and y are the same before doing the operation. 
...
row_col_dot_matmat_out <- row_col_dot_matmat(A, B)
base_R_matmat_out <- A %*% B
all.equal(row_col_dot_matmat_out,base_R_matmat_out)
```
...
all.equal(col_oriented_matmat_out,base_R_matmat_out)
...
***Answer:***
`col_oriented_matmat` is faster than `row_col_dot_matmat` (~180ms vs ~14s !) because R stores matrices in a column manner; therefore it is faster to access elements in a column-oriented way. 
...
***Answer:***
`col_oriented_matmat` is faster than `A %*% B` (~330ms). 
...
***Answer:***
I created Makevars file inside ~/.R and selectively turned on and off SSE, AVX, and FMA.
With the default mode, -O2 was already used as this resulting lines:
"/Library/Frameworks/R.framework/Resources/bin/R CMD SHLIB -o 'sourceCpp_4.so' 'gemm_eigen.cpp' 
clang++ -arch arm64 -std=gnu++14 -I"/Library/Frameworks/R.framework/Resources/include" -DNDEBUG   -I"/Library/Frameworks/R.framework/Versions/4.2-arm64/Resources/library/Rcpp/include" -I"/Library/Frameworks/R.framework/Versions/4.2-arm64/Resources/library/RcppEigen/include" -I"/Users/puwanatsang/Documents/PH.140.778.SP23/stats-comp-algo-software-2023/lecture/src" -I/opt/R/arm64/include   -fPIC  -falign-functions=64 -Wall -g -O2  -c gemm_eigen.cpp -o gemm_eigen.o
clang++ -arch arm64 -std=gnu++14 -dynamiclib -Wl,-headerpad_max_install_names -undefined dynamic_lookup -single_module -multiply_defined suppress -L/Library/Frameworks/R.framework/Resources/lib -L/opt/R/arm64/lib -o sourceCpp_4.so gemm_eigen.o -F/Library/Frameworks/R.framework/.. -framework R -Wl,-framework -Wl,CoreFoundation"
The relative speed from the fastest to slowest is as follows: `dgemm_eigen` (56ms), `col_oriented_matmat` (183ms), and `row_col_dot_matmat` (13s).
I also observed that when I took the charger off, the speed got slower. 
Next, I tried `CXXFLAGS += -mno-sse`, `CXXFLAGS += -msse`, `CXXFLAGS += -msse4`, `CXXFLAGS += -mavx`, `CXXFLAGS += -mfma`, and `CXXFLAGS += -mavx -mfma`.
However, the speeds of the three algorithms stayed relatively the same. This happened because I used Macbook M2 chip, and the optimizers only worked with intel chips.
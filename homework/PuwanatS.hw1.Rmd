...
**Your answer:** The size of the git_experiment folder after commiting foo.pdf files is much larger (~8M) than the sizes of the individual files inside it (just small Kb !). This is because pdf files in .git hidden folder are binary (large) files. If we use this command "du -lha" in the git_experiment directory, we will see a lot of the small files come from the pdf files. 
...
Hyperlink to the `hiperglm` repository on your Github account: https://github.com/puwanat-s/hiperglm.git
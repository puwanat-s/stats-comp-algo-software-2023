...
Answer: The result of sample_var_with_question_mark is -147,456 with the given seed. This variance estimate does not make sense because rnorm(1) has sd = 1, so the estimate 
should be around 1. Indeed, if the constant number in theta (1e10) is close to the sd (e.g.theta = 1e3 + rnorm(1)), the estimate would get close to 1, which is the sd^2, 
and this result stays true even when there is no fixed seed. The estimate fluctuates with a huge range when you increase or decrease the seed (for the case of theta = 1e10 + rnorn(1)). 
Still, it is not close to 1. Without a fixed seed, the distribution of the estimator sample_var_with_question_mark looks like a normal distribution with a median at 0 
(I tried running without a fixed seed 1,000 times and the range of the estimator is between -6e+05 and 6e+05). 
...
Answer for part A: From the class lecture, the error associated with the centered difference method is (step size)^2. So, for a plot of the relative error and the step size in log-log scale, 
the expected slope should be 2. The relative error plot does not 100% agree with what the mathematical theory predicts because the plot is V-shape, not a straight line. 
There is a critical point where, passing that point, the relative error increases with smaller step sizes. Sure enough, the slope of the first part is close to 2 
(pick 2 points of the relative errors and step sizes before the critical value and directly calculate the slope). The slope in the "latter part" though is close to -1
(pick 2 points of the relative errors and step sizes after the critical value). This happens because there is a limitation in .Machine$double.eps (at ~2e-16). 
If the limitation is included in the relative error calculation, the relative error would be bound with (machine error/step size) + (step size)^2. So when the step size is small,
the relative error is bound by (step_size)^2 term, hence the first part of the V-shaped plot. However, when the step size is very small, the term (machine error/step size) becomes
dominant, hence the latter part of the plot. 
...
Link to the pull request: https://github.com/puwanat-s/hiperglm/pull/1
